...
def helper_new_grid(h: int, w: int) -> List[List[List[Optional[Character]]]]:
    """Make a new blank grid based on the given width and height
    """
    grid = []
    for row in range(h):
        grid.append([])
...
def helper_add_raccoon(c: Character,
                       rac_list: List[Raccoon]) -> None:
    """Checks if the character is a raccoon not in a can, if it is, then
    appends it to the list of raccoon
    """
    if c and isinstance(c[0], Raccoon):
        rac_list.append(c[0])
...
class GameBoard:
    ...
    _grid: List[List[List[Optional[Character]]]]
    ...
    def __init__(self, w: int, h: int) -> None:
        ...
        self._player = None
        self._grid = helper_new_grid(h, w)
    ...
    def place_character(self, c: Character) -> None:
        ...
        True
        """
        if self._grid[c.y][c.x] and self._grid[c.y][c.x][0].get_char() == 'O' \
    ...
    def to_grid(self) -> List[List[chr]]:
        ...
        [['P', '-', '-'], ['-', 'R', 'C']]
        """
        game_state = helper_new_grid(self.height, self.width)
    ...
    def move_char(self, c: Character, new_pos: Tuple[int, int]) -> None:
        """Moves the character to the new position.

        Precondition:
        - Character <c> is on board
        - new_pos is on the board
        - Character <c> can move to new position
        ...
        True
        """
        self._grid[c.y][c.x].remove(c)
        self._grid[new_pos[1]][new_pos[0]].append(c)
    ...
    def check_game_end(self) -> Optional[int]:
        ...
        for i in range(self.height):
            for j in range(self.width):
                if self._grid[i][j] and isinstance(self._grid[i][j][0],
                                                   Raccoon) \
                        and self._grid[i][j][0].check_trapped():
                    raccoons_trapped += 1
                elif self._grid[i][j] and isinstance(self._grid[i][j][0],
                                                     Raccoon):
                    self.ended = False
                    return None
        self.ended = True
    ...
    def adjacent_bin_score(self) -> int:
       ...
       >>> print(b)
        ...
        for i in self._grid:
            for j in i:
                if j and isinstance(j[0], RecyclingBin) and (j[0].x, j[0].y) \
                        not in lst:
                    adj = self._helper_adjacent_bins((j[0].x, j[0].y), [])
                    size = max(size, len(adj))
                    lst.extend(adj)
        return size

    def _helper_adjacent_bins(self, pos: Tuple[int, int],
                              lst: List[Tuple[int, int]]) \
            -> List[Tuple[int, int]]:
        """Counts the adjacent recycling bin clusters.

        Precondition:
        - pos is on the grid
        - pos is the position of a recycling bin
        - each element in lst is a valid position on the board
        >>> b = GameBoard(3, 3)
        >>> _ = RecyclingBin(b, 1, 1)
        >>> _ = RecyclingBin(b, 0, 0)
        >>> _ = RecyclingBin(b, 2, 2)
        ...
        for direction in DIRECTIONS:
            ...
            if self.on_board(new_pos[0], new_pos[1]) and new_pos not in lst:
                chars = self.at(new_pos[0], new_pos[1])
                if chars and isinstance(chars[0], RecyclingBin):
                    bins.extend(self._helper_adjacent_bins(new_pos, lst))
        return bins
...
class Character:
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        self.board.move_char(self, new_pos)
        self.x = new_pos[0]
        self.y = new_pos[1]
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        """
        new_pos = (self.x + direction[0], self.y + direction[1])
        if self.board.on_board(new_pos[0], new_pos[1]):
            chars = self.board.at(new_pos[0], new_pos[1])
            if not chars or (chars[0].get_char() == 'B'
                             and chars[0].move(direction)):
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        True
        """
        nbs = get_neighbours((self.x, self.y))
        for i in nbs:
            if self.board.on_board(i[0], i[1]):
                chars = self.board.at(i[0], i[1])
                if not chars or \
                        (len(chars) == 1 and isinstance(chars[0], GarbageCan)):
                    return False
        return True
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        """
        new_pos = (self.x + direction[0], self.y + direction[1])
        if not self.inside_can and \
                self.board.on_board(new_pos[0], new_pos[1]):
            chars = self.board.at(new_pos[0], new_pos[1])
            if not chars:
                Character.move(self, direction)
                return True
            elif len(chars) == 1 and isinstance(chars[0], GarbageCan):
                if chars[0].locked:
                    chars[0].locked = False
                else:
                    Character.move(self, direction)
    ...
    def take_turn(self) -> None:
        ...
        if not self.inside_can:
            not_blocked = DIRECTIONS.copy()
            rnd_char = random.randrange(len(not_blocked))
            while not self.move(not_blocked[rnd_char]):
                not_blocked.pop(rnd_char)
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        if not self.inside_can:
            for direction in DIRECTIONS:
                new_pos = (self.x + direction[0], self.y + direction[1])
                move_pos = self._helper_move(direction, new_pos)
                if self.x != move_pos[0] or self.y != move_pos[1]:
                    gb2 = abs(move_pos[0] - self.x) + abs(move_pos[1] - self.y)
                    garbage_cans1.append(direction)
                    garbage_cans2.append(gb2)
            if garbage_cans1:
                closest = garbage_cans2.index(min(garbage_cans2))
                Raccoon.move(self, garbage_cans1[closest])
            else:
                Raccoon.take_turn(self)

    def _helper_move(self, direction: Tuple[int, int],
                     move_pos: Tuple[int, int]) -> Tuple[int, int]:
        """Check if there is an unblocked garbage can at move_pos, if not then
        searching for one in the given direction.

        >>> b = GameBoard(8, 3)
        >>> s = SmartRaccoon(b, 3, 0)
        >>> _ = GarbageCan(b, 3, 2, False)
        >>> s._helper_move(DOWN, (3, 1))
        (3, 2)
        """
        if self.board.on_board(move_pos[0], move_pos[1]):
            next_chars = self.board.at(move_pos[0], move_pos[1])
            if len(next_chars) == 1 and isinstance(next_chars[0], GarbageCan):
                return move_pos
            elif not next_chars or isinstance(next_chars[0], Player):
                new_pos = (move_pos[0] + direction[0],
                           move_pos[1] + direction[1])
                return self._helper_move(direction, new_pos)
